...
public class CardService {
    ...
    public CardService(BankCardRepository cardRepository, CustomerService customerService) {
    ...
    private BankCard findOwnedCard(Long userId, Long cardId) {
        Long customerId = customerService.requireCustomerIdForUser(userId);
        return cardRepository.findByCardIdAndCustomerId(cardId, customerId)
                .orElseThrow(() -> new ResourceNotFoundException("Card was not found."));
    }

    private static CardResponse toResponse(BankCard card) {
        return new CardResponse(
                card.getCardId(), card.getAccountId(), card.maskedNumber(),
                card.getCardType().name(), card.getCardNetwork().name(),
                card.getExpiryMonth(), card.getExpiryYear(), card.getCardStatus().name(),
                card.getActivatedAt());
    }
}
