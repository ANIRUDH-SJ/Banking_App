...
public class AccountService {
    ...
    public List<AccountSummaryResponse> getAccountsForUser(Long userId) {
        ...
        return bankAccountRepository.findAllById(accountHolderRepository.findActiveAccountIdsByCustomerId(customerId))
                .stream()
    ...
    public void requireOwnership(Long userId, Long accountId) {
        Long customerId = customerService.requireCustomerIdForUser(userId);
        if (!accountHolderRepository.existsByAccountIdAndCustomerIdAndIsActive(accountId, customerId, "Y")) {
    ...
    private AccountSummaryResponse toResponse(BankAccount account) {
        return new AccountSummaryResponse(
                account.getAccountId(), account.getBranchId(), account.getAccountNumber(), account.getAccountType(),
                account.getCurrencyCode(), account.getAccountStatus(), account.getCurrentBalance(),
