...
public class BranchService {
    ...
    public List<BranchResponse> getActiveBranches(Long bankId) {
        ...
        return branchRepository.findByBankIdAndIsActiveOrderByBranchNameAsc(bankId, "Y")
                .stream()
                .map(this::toResponse)
                .toList();
    }
    ...
    private BranchResponse toResponse(Branch branch) {
        return new BranchResponse(
                branch.getBranchId(), branch.getBankId(), branch.getBranchCode(), branch.getBranchName(),
                branch.getIfscCode(), branch.getCity(), branch.getState());
    }
}
