...
public class OtpService {
    ...
    public OtpService(OtpVerificationRepository repository, PasswordEncoder passwordEncoder,
                      OtpDeliveryService deliveryService, AppUserRepository userRepository,
        ...
        this.repository = repository; this.passwordEncoder = passwordEncoder; this.deliveryService = deliveryService;
        this.userRepository = userRepository; this.verificationAttemptService = verificationAttemptService;
        this.expiry = Duration.ofMinutes(expiryMinutes); this.maxIssuesPerWindow = maxIssuesPerWindow;
    ...
    public Challenge issue(AppUser user, OtpPurpose purpose, String intentDigest) {
        userRepository.findByIdForUpdate(user.getUserId())
                .orElseThrow(() -> new UnauthorizedException("User is not eligible for an OTP challenge."));
        Instant now = Instant.now();
        if (intentDigest != null) {
            long recentIssues = repository.countByUserIdAndPurposeAndCreatedAtAfter(
                    user.getUserId(), purpose, now.minus(issueWindow));
            if (recentIssues >= maxIssuesPerWindow) {
                throw new IllegalStateException("Too many OTP challenges have been requested. Please try again later.");
            }
        ...
        repository.saveAndFlush(new OtpVerification(user.getUserId(), challengeId,
                passwordEncoder.encode(code), purpose, now.plus(expiry), intentDigest));
    ...
    public void verifyForUser(Long userId, String challengeId, String code, OtpPurpose purpose,
                              String intentDigest) {
        OtpVerificationAttemptService.Result result = verificationAttemptService.verify(
                userId, challengeId, code, purpose, intentDigest);
        switch (result) {
            case VERIFIED -> { return; }
            case EXPIRED -> throw new UnauthorizedException("OTP challenge has expired.");
            case INTENT_MISMATCH -> throw new UnauthorizedException("OTP challenge does not match this request.");
            case INVALID, INVALID_CODE -> throw new UnauthorizedException("OTP challenge or code is invalid.");
        }
    }
