...
public class OtpVerificationAttemptService {
    ...
    public OtpVerificationAttemptService(OtpVerificationRepository repository,
    ...
    public Result verify(Long userId, String challengeId, String code, OtpPurpose purpose,
                         String intentDigest) {
        OtpVerification otp = repository.findByChallengeIdForUpdate(challengeId).orElse(null);
        if (otp == null || !otp.getUserId().equals(userId) || otp.getPurpose() != purpose
    ...
    public enum Result { VERIFIED, INVALID, EXPIRED, INTENT_MISMATCH, INVALID_CODE }
}
