...
public class LoanService {
    ...
    public LoanResponse getLoan(Long userId, Long loanId) {
        Long customerId = customerService.requireCustomerIdForUser(userId);
        return loanRepository.findByLoanIdAndCustomerId(loanId, customerId)
                .map(LoanService::toResponse)
    ...
    public static LoanResponse toResponse(Loan loan) {
        return new LoanResponse(
                loan.getLoanId(), loan.getLoanAccountNumber(), loan.getLoanType().name(),
                loan.getPrincipalAmount(), loan.getOutstandingPrincipal(), loan.getInterestRate(),
                loan.getTermMonths(), loan.getEmiAmount(), loan.getCurrencyCode().trim(),
                loan.getDisbursedOn(), loan.getNextDueDate(), loan.getMaturityDate(),
