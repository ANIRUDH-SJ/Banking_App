...
public class AppUser {
    ...
    @Enumerated(EnumType.STRING) @Column(name = "account_status", nullable = false)
    ...
    public boolean hasRole(String roleCode) { return roles.stream().anyMatch(role -> roleCode.equals(role.getRoleCode())); }
    public boolean isLockedAt(Instant now) { return accountStatus == UserStatus.LOCKED && lockedUntil != null && lockedUntil.isAfter(now); }
    public void recordFailedLogin(int maxAttempts, Instant lockUntil) {
        failedLoginAttempts++;
        if (failedLoginAttempts >= maxAttempts) { accountStatus = UserStatus.LOCKED; lockedUntil = lockUntil; }
    }
    public void recordSuccessfulLogin(Instant now) { failedLoginAttempts = 0; lockedUntil = null; lastLoginAt = now; }
    public void unlock() { accountStatus = UserStatus.ACTIVE; failedLoginAttempts = 0; lockedUntil = null; }
