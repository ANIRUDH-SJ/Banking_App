...
public class BeneficiaryService {
    ...
    public BeneficiaryResponse create(Long userId, BeneficiaryRequest request) {
        ...
        String ifsc = request.ifscCode().trim().toUpperCase();
        if (repository.existsByCustomerIdAndNicknameIgnoreCase(customerId, nickname)
    ...
    public List<BeneficiaryResponse> list(Long userId) {
        return repository
                .findAllByCustomerIdOrderByNicknameAsc(
                        customerService.requireCustomerIdForUser(userId))
                .stream()
                .map(this::toResponse)
                .toList();
    }
    ...
    public BeneficiaryOtpChallengeResponse issueActivationOtp(Long userId, Long beneficiaryId) {
        ...
        return new BeneficiaryOtpChallengeResponse(
                otpService
                        .issue(
                                userId,
    ...
    public BeneficiaryResponse activate(
            Long userId, Long beneficiaryId, BeneficiaryActivationRequest request) {
        Beneficiary beneficiary = owned(userId, beneficiaryId);
        beneficiary.requirePendingForActivation();
        requireCoolingPeriod(beneficiary);
        otpService.authorize(
    ...
    @Transactional(readOnly = true)
    public Beneficiary requireActiveOwned(Long userId, Long beneficiaryId) {
        Beneficiary b = owned(userId, beneficiaryId);
        if (!b.isActive()) throw new IllegalStateException("Beneficiary is not active.");
        return b;
    }

    private Beneficiary owned(Long userId, Long id) {
        return repository
                .findByBeneficiaryIdAndCustomerId(
                        id, customerService.requireCustomerIdForUser(userId))
                .orElseThrow(() -> new ResourceNotFoundException("Beneficiary was not found."));
    }
    ...
    private void requireCoolingPeriod(Beneficiary beneficiary) {
        if (beneficiary.getCreatedAt().plus(activationCooldown).isAfter(Instant.now()))
            throw new IllegalStateException(
                    "Beneficiary activation is available after the cooling period.");
    }
